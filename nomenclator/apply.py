    ...
    def __init__(self, scan_result_path: str):
        ...
        try:
            ...
            raise FileNotFoundError(
                f"Scan result file not found: {scan_result_path}"
            )
        except json.JSONDecodeError as e:
            raise ValueError(
                f"Invalid JSON in scan result file: {e}"
            )
            ...
            for item in items:
                ...
                if 0 <= line_num < len(lines):
                    ...
                    if old_line != new_line:
                        old_escaped = re.escape(item['old_name'])
                        new_name = item['new_name']
                        sed_cmd = (
                            f"    sed -i '' '{line_num + 1}s/{old_escaped}/"
                            f"{new_name}/' {file_path}\n"
                        )
                        output += sed_cmd
